...
class Rinci extends \Aksara\Laboratory\Core
{
	...
	public function index()
	{
		...
		{
            if(empty(service('request')->getPost('kegiatan')))
            {
                return throw_exception(403, 'Bidang Rekening Kegiatan dibutuhkan ...');
            }

            list($r1, $r2, $r3, $r4, $r5, $r6, $r7)	= array_pad(explode('.', service('request')->getPost('kegiatan')), 5, 0);
            list($r8, $r9, $r10, $r11, $r12)	    = array_pad(explode('.', service('request')->getPost('belanja')), 5, 0);
		...
		(
			array
			(
				'bendahara/spp/ls'					=> 'Bendahara Pengeluaran Spp Ls',
				'../ls'								=> 'Spp Ls'
			)
		);
		
		$this->set_title('Rinc SPP Langsung')
		...
		(
			array
			(
				'usulan'							=> 'callback_validasi_nilai'
			)
		)
		->set_relation
		(
