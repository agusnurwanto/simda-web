...
<html>
    ...
    <table class="table">
        ...
        <tr>
            ...
            <th class="border text-sm-2" width="47%">
                URAIAN
            </th>
            <th class="border text-sm-2" width="20%">
                NILAI STS
        ...
        <tr>
		    ...
		    {
			    $nilai						        = (($val->Nilai_STS) ? $val->Nilai_STS : null);
			    $jumlah_nilai				        += $nilai;
			    if($val->No_STS != $no_sts)
			    {
				    echo '
                        <tr>
                            <td class="border text-sm text-center no-border-top no-border-bottom">';
                            if($val->Tgl_STS != $lewat_tanggal)
                            {
                                echo date_indo($val->Tgl_STS);
                            }
                            echo '
                            </td>
                            ...
                            <td class="border text-sm text-right no-border-top no-border-bottom">
                                ' . number_format_indo($val->Nilai_STS, 2) . '
                            </td>
                        </tr>
                    ';
			    }
			    $no_sts					            = $val->No_STS;
			    $lewat_tanggal                      = $val->Tgl_STS;
			    $num++;
		    }
		    ?>
        </tr>
