					...
					foreach($results['data'] as $key => $val)
					{
						...
						if($val->id_program != $id_program)
						{
							...
							echo '
								<tr>
									<td class="border text-sm">
										<b>' . $val->kd_urusan . '.' . $val->kd_bidang . '.' . sprintf('%02d', $val->kd_prog) . '</b>
									</td>
						...
						{
							$jumlah_belanja_sub_kegiatan	=	$val->belanja_operasi_kegiatan + $val->belanja_modal_kegiatan + $val->belanja_tidak_terduga_kegiatan + $val->belanja_transfer_kegiatan;
							echo '
								<tr>
									<td class="border text-sm">
										' . $val->kd_urusan . '.' . $val->kd_bidang . '.' . sprintf('%02d', $val->kd_prog) . '.' . sprintf('%02d', $val->kd_keg) . '
									</td>
									...
									<td class="border text-right text-sm">
										' . number_format_indo($jumlah_belanja_sub_kegiatan) . '
									</td>
									<td class="border text-right text-sm">
				...
				<tfoot>
					<tr>
						<td class="border text-sm no-border-right" colspan="3">
							<i>
								RKA Belanja SKPD - <?php echo (isset($results['header']->nm_unit) ? $results['header']->nm_unit : '-'); ?>
